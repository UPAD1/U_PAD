...
async def upload_document(request: Request, file: UploadFile = File(...), db: Session = Depends(database.get_db)):
    ...
    deidentified_text = dlp_result.get("deidentified_text", text)
...
async def upload_image(request: Request, file: UploadFile = File(...), db: Session = Depends(database.get_db)):
    ...
    deidentified_text = dlp_result.get("deidentified_text", extracted_text)


    return JSONResponse(content= {
        
        "img_path": "http://localhost:8000/" + masked_path.replace("\\", "/"),
        "text": extracted_text,
        "findings": findings,
        "deidentified_text": deidentified_text
    })
